...
def home(request):
	...
	# Vérifier si l'on s'est connecté
	if request.method == 'POST':
		...
		if user is not None:
			...
			messages.success(request, "Il y a eu une erreur de connexion, veuillez réessayer...")
			return redirect('home')
	else:
...
def delete_record(request, pk):
	if request.user.is_authenticated:
		...
		messages.success(request, "Vous devez être connecté pour faire cela...")
		return redirect('home')
	...
	if request.user.is_authenticated:
		if request.method == "POST":
			if form.is_valid():
				...
				messages.success(request, "Etudiant ajouté...")
				return redirect('home')
